...
def complete_mail(
    ...
    @type sender: tuple
    @param sender: a tuple of the form (u'Seer Name', 'sender.address@domain.com')
    ...
    rcpt_to = [getaddr(item) for item in itertools.chain(recipients, cc, bcc)]
...
def send_mail(
    ...
    try:
        ...
        error = f"all recipients refused: {', '.join(e.recipients.keys())}"
